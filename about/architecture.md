...
Instead, Clockwork uses a **delegated signatory** model. When a worker builds and submits a crank transaction, the Clockwork program receives it, verifies it, and adds an additional PDA signer before forwarding the request on to the target program.&#x20;
