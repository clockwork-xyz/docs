...
Instead, Clockwork uses a **delegated signatory** model. When a worker submits a crank transaction, the Clockwork program receives it first, verifies the request, and adds an additional PDA signer to the transaction before forwarding it on to the target program.&#x20;
